...
- **Yeni dosya `SessionChartDrawer.java`**: Seans bantlari (PRE mavi / MARKET yesil / AFTER mor), kesikli ayraclar, noktali baseline (dunku kapanis), dolgulu NAV egrisi, ucunda beyaz nokta, altta yerel saatle 4 saat etiketi.
- **Cizgi rengi = gauge ibresinin rengi (1 Agu 2026)**: NAV cizgisi artik sabit yesil/kirmizi DEGIL; her nokta o degerin gauge yayindaki rengiyle ciziliyor (`GaugeDrawer.colorForPct()` - ibre, degisim yazisi ve grafik AYNI fonksiyonu cagirir, tek kaynak). Cizim `LinearGradient` shader ile yatay renk gecisi olarak yapilir (180 stop); cizgi ve dolgu ayni stop'lari kullanir. Skala BIREBIR gauge ile ayni (±%4) - kullaniciya "tipik gunlerde sari agirlikli kalir" uyarisi yapildi, bilerek secildi. Pratikte ±%1 asan gunlerde turuncu/sari-yesil net gorunuyor (dogrulandi: -%1.32 dip turuncu, +%0.99 zirve sari-yesil).
- **Min/max cizgileri NOTR gri**: NAV cizgisi sari tonlarda kaldigi icin amber min/max cizgileriyle karisiyordu; `COL_HILO` gri yapildi.
- **VIX overlay**: Sonuk ince mavi cizgi (`COL_VIX = 0x8A60A5FA`), NAV egrisinin ALTINDA cizilir. VIX (~16-25) NAV yuzdesiyle (±0.5) ayni eksende gosterilemez -> KENDI gun ici min/max araligina gore olceklenir (plot'un %88'i). Cizgi VIX'in YONUNU/SEKLINI gosterir, dikey konumu NAV ile kiyaslanamaz. Veri zaten `^VIX` quote'unda vardi. `vix_series` prefs'te `preStart;v0,...` olarak saklanir; `parseRaw()` preStart eslesmezse null doner (onceki gunden kalma cizgi cizilmesin).
- **Gun ici min/max cizgileri**: Amber kesikli iki yatay cizgi gunun en dusuk/en yuksek NAV'ini isaretler, sag ucta $ degeri yazar (orn `$950,847` / `$929,117`). $ yazabilmek icin `prev_close_nav` prefs'e ayrica kaydedilir (`prevCloseNav` NaN ise yuzde yazilir). Anlik yuzde etiketi KALDIRILDI - gauge'da zaten var, tekrar oluyordu. Y ekseni nefes payi 1.18 -> 1.34 (etiketler cizgilerin disina sigsin), yer yoksa etiket cizginin ote yanina gecer.
